...
public class ShopController : MonoBehaviour
{
    ...
    public IEnumerator RunShop(EventManager manager)
    {
        ...
        for ( int i = 0; i < shopPotionCount; i++ )
        {
            ...
            if ( potionIndex == 0 )
            {
                item.GetComponent<ShopItem>().Init(healthPotionSprite, 50, ShopItemType.Potion, this);
            ...
            {
                item.GetComponent<ShopItem>().Init(maxHealthPotionSprite, 75, ShopItemType.Potion, this);
                item.GetComponent<ShopItem>().onItemPurchased += () => {
            ...
            {
                item.GetComponent<ShopItem>().Init(attackPotionSprite, 75, ShopItemType.Potion, this);
                item.GetComponent<ShopItem>().onItemPurchased += () => {
        ...
        for ( int i = 0; i < shopRelicCount; i++ )
        {
            ...
            var relic = relics[i];
            item.GetComponent<ShopItem>().Init(relic.icon, 100, ShopItemType.Relic, this);
            item.GetComponent<ShopItem>().onItemPurchased += () =>{
